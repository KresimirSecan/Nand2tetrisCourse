class Parser:
    ...
    def _parseLine(self, line, n):
        ...
        if l[0] == "push": 
            ...
            if len(l) == 3:
                if ( not(l[2].isdigit())) :	
                    self._flag = False
                    Parser._error("in line ",n,"Undifend pop argument "+l[2])
                    return ""
                else:
                    return "//" + " ".join(l) + "\n" + self._pop(l[1], l[2], n)
            else:
    ...
    def _label(self, lab, n):
        self._labellist.append(lab)
        return "(" + self._func + "$" + lab + ")"
    
    def _goto(self, lab, n):
        self._labeljumplist.append(lab)
        return "@" + self._func + "$" + lab + "\n0; JMP"
    ...
    def _writeFile(self):
        #check labels
        for i in self._labeljumplist:
            if i not in self._labellist:
                self._flag = False
                Parser._error("Unvalid jump to "+i)
                return ""
